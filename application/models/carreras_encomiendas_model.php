...
class Carreras_encomiendas_model extends CI_Model
{
    ...
    public function getEncomiendasPorTaxista($id_usu,$servicio)
    {
        ...
        // Ejecuta la consulta
        $query = $this->db->query("
            SELECT ce.*, v.placa_veh, v.marca_veh, v.anio_veh, socio.nombres AS socio_nombres, socio.apellidos AS socio_apellidos, socio.correo AS socio_correo, socio.telefono AS socio_telefono, socio.direccion AS socio_direccion, solicitante.nombres AS solicitante_nombres, solicitante.apellidos AS solicitante_apellidos, solicitante.correo AS solicitante_correo, solicitante.telefono AS solicitante_telefono, solicitante.direccion AS solicitante_direccion FROM carreras_encomiendas ce INNER JOIN vehiculos v ON ce.fk_car_veh = v.id_veh INNER JOIN usuarios socio ON v.fk_veh_usu = socio.id_usu INNER JOIN usuarios solicitante ON ce.fk_car_usu = solicitante.id_usu WHERE ce.tipo_ce = '$servicio' AND socio.id_usu = $id_usu AND ce.fecha_entrega  >= CURDATE() AND ce.estadoCarrera IN ('por aceptar', 'aceptado');
      ...
      public function getCarrerasPorTaxistaRial_time($id_usu,$servicio)
    {
        // Asegúrate de que $id_usu es un entero para prevenir inyecciones SQL
        $id_usu = (int)$id_usu;
        ...
        // Ejecuta la consulta
        $query = $this->db->query("
            SELECT ce.*, v.placa_veh, v.marca_veh, v.anio_veh, socio.nombres AS socio_nombres, socio.apellidos AS socio_apellidos, socio.correo AS socio_correo, socio.telefono AS socio_telefono, socio.direccion AS socio_direccion, solicitante.nombres AS solicitante_nombres, solicitante.apellidos AS solicitante_apellidos, solicitante.correo AS solicitante_correo, solicitante.telefono AS solicitante_telefono, solicitante.direccion AS solicitante_direccion FROM carreras_encomiendas ce INNER JOIN vehiculos v ON ce.fk_car_veh = v.id_veh INNER JOIN usuarios socio ON v.fk_veh_usu = socio.id_usu INNER JOIN usuarios solicitante ON ce.fk_car_usu = solicitante.id_usu WHERE ce.tipo_ce = '$servicio' AND socio.id_usu = $id_usu AND ce.fecha_carrera >= CURDATE() AND ce.estadoCarrera IN ('por aceptar', 'aceptado');
        ", array($id_usu)); // Usa parámetros para prevenir inyecciones SQL
    ...
    public function carrerasPasadas($id_usu)
    {
        // Asegúrate de que $id_usu es un entero para prevenir inyecciones SQL
        $id_usu = (int)$id_usu;
        ...
        // Ejecuta la consulta
        $query = $this->db->query("
          SELECT ce.*, v.placa_veh, v.marca_veh, v.anio_veh, socio.nombres AS socio_nombres, socio.apellidos AS socio_apellidos, socio.correo AS socio_correo, socio.telefono AS socio_telefono, socio.direccion AS socio_direccion, solicitante.nombres AS solicitante_nombres, solicitante.apellidos AS solicitante_apellidos, solicitante.correo AS solicitante_correo, solicitante.telefono AS solicitante_telefono, solicitante.direccion AS solicitante_direccion FROM carreras_encomiendas ce INNER JOIN vehiculos v ON ce.fk_car_veh = v.id_veh INNER JOIN usuarios socio ON v.fk_veh_usu = socio.id_usu INNER JOIN usuarios solicitante ON ce.fk_car_usu = solicitante.id_usu WHERE socio.id_usu = $id_usu AND ce.fecha_entrega < CURDATE() OR ce.estadoCarrera IN ('RECHAZADO', 'FINALIZADO', 'CANCELADO');
       " , array($id_usu));// Verifica si hay resultados
        if ($query->num_rows() > 0) {
            return $query->result(); // Devuelve un array de objetos
        } else {
            return []; // Devuelve un array vacío si no hay resultados
