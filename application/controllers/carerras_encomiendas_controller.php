...
class Carerras_encomiendas_controller extends CI_Controller
{
    ...
    public function reporteMisCarreras()
    {
        if (
            // $this->session->userdata("conectado")->perfil == "ADMINISTRADOR" ||
            $this->session->userdata("conectado")->perfil == "PRESIDENTE" ||
            ...
            $data["carreras"] = $this->Carreras_encomiendas_model->getCarrerasPorTaxistaRial_time($id_usu,"CARRERA");
    ...
    public function reporteMisEncomiendas()
    {
        if (
            // $this->session->userdata("conectado")->perfil == "ADMINISTRADOR" ||
            $this->session->userdata("conectado")->perfil == "PRESIDENTE" ||
            ...
            $id_usu= $this->session->userdata("conectado")->id_usu;
            $data["carreras"] = $this->Carreras_encomiendas_model->getEncomiendasPorTaxista($id_usu,"ENCOMIENDA");
    ...
    public function reportesCarrerasPasadas()
    {
        if (
            // $this->session->userdata("conectado")->perfil == "ADMINISTRADOR" ||
            $this->session->userdata("conectado")->perfil == "PRESIDENTE" ||
            ...
            $this->load->view("administracion/header");
            $this->load->view("car_enc/reporteMisEncomiendaspasadas", $data);
            $this->load->view("administracion/footer");
        }
    }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->Carreras_encomiendas_model->insertar($data)) {
                    ...
                    if ($telefono_taxista) {
                        ...
                        $from = 'whatsapp:+19787432455'; // Número de WhatsApp registrado en Twilio (debe incluir el prefijo "whatsapp:")
    ...
    public function Aceptar($id_car)
    {
        ...
        if ($this->Carreras_encomiendas_model->status($nuevoStatusCarrera, $id_car)) {
         ...
         redirect("Carerras_encomiendas_controller/reporteMisEncomiendas/");
    }
    ...
    public function Finalizado($id_car)
    {
        $id_usu = $this->session->userdata("conectado")->id_usu;
        $nuevoStatusCarrera = "FINALIZADO";

        if ($this->Carreras_encomiendas_model->status($nuevoStatusCarrera, $id_car)) {
            $this->session->set_flashdata('actualizar', "La encomienda se entrego exitosamente");
        } else {
            $telefono_solicitante = $this->Carreras_encomiendas_model->obtenerTelefonoSolicitanteEncomienda($id_car);
            $telefono_taxista = $this->session->userdata("conectado")->telefono;
    ...
    public function AceptarCarrera($id_car)
{
    $id_usu = $this->session->userdata("conectado")->id_usu;
    $nuevoStatusCarrera = "ACEPTADO";
    $status_veh = "OCUPADO";
    $id_veh = $this->vehiculo_model->obtener_vehiculo($id_usu);
    
    if (is_object($id_veh) && isset($id_veh->id_veh)) {
        $id_veh = $id_veh->id_veh;
       
    }
    if (!$this->Carreras_encomiendas_model->status($nuevoStatusCarrera, $id_car)) {
        
        $this->session->set_flashdata('actualizar', "Se aceptó la carrera correctamente");
        
        

        if(!$this->vehiculo_model->status($status_veh, $id_veh)){
           
            $this->session->set_flashdata("actualizar", "Su estado cambió a ocupado. Cuando termine el servicio, recuerde que debe dar clic en el botón 'FINALIZADO'.");
            
        } else {
            $this->session->set_flashdata("error", "Su estado no cambió a ocupado. por alguna razon.");
            
        }
    } else {
        log_message('error', 'Error al actualizar el estado de la carrera con ID: ' . $id_car);
        ...
        if ($telefono_taxista) {
            ...
            $to = 'whatsapp:' . $telefono_solicitante;
            $from = 'whatsapp:+19787432455';
            $body = 'La encomienda encargada ha sido aceptada, revisa los detalles en la aplicación.';
      ...
      redirect("Carerras_encomiendas_controller/reporteMisCarreras/");
}
public function RechazarCarrera($id_car)
{
    $id_usu = $this->session->userdata("conectado")->id_usu;
    $nuevoStatusCarrera = "RECHAZADO";
    $status_veh = "LIBRE";
    $id_veh = $this->vehiculo_model->obtener_vehiculo($id_usu);
    ...
    if (!$this->Carreras_encomiendas_model->status($nuevoStatusCarrera, $id_car)) {
        ...
        if(!$this->vehiculo_model->status($status_veh, $id_veh)){
            ...
            $this->session->set_flashdata("actualizar", "Se rechazo con exito la carrera su estado sige libre y los usuarios lo pueden ver en el mapa.");
...
public function FinalizarCarrera($id_car)
{
    $id_usu = $this->session->userdata("conectado")->id_usu;
    $nuevoStatusCarrera = "FINALIZADO";
    $status_veh = "LIBRE";
